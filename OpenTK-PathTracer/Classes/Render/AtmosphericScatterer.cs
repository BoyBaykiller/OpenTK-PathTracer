...
namespace OpenTK_PathTracer.Render
{
    class AtmosphericScattering : RenderEffectBase
    {
        ...
        public AtmosphericScattering(int size, int inScatteringSamples, int densitySamples, float scatteringStrength, float densityFallOff, float atmossphereRadius, Vector3 waveLengths, Vector3 lightPos)
        {
            ...
            Program = new ShaderProgram(new Shader(ShaderType.ComputeShader, "Src/Shaders/AtmosphericScattering/compute.comp".GetPathContent()));
            bufferObject = new BufferObject(BufferRangeTarget.UniformBuffer, 3, Vector4.SizeInBytes * 4 * 7 + Vector4.SizeInBytes, BufferStorageFlags.DynamicStorageBit);
